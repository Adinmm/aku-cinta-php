...
class Routes
{
    ...
    public function _init()
    {
        ...
        $_str = str_replace($_SERVER['SCRIPT_NAME'], '', $this->_uri);
        if ($_str) {
            if ($_pos = strpos($_str, '?'))
                $_str = substr($_str, 0, $_pos);
            $_tmp = explode(DS, $_str);
            array_shift($_tmp);
            $this->_depths = array_filter($_tmp, 'strlen');
        }
        return $this;
    }
    ...
    public function _depth($x = 0, $camel_case = false)
    {
        ...
        return $camel_case ? Helpers::_camel_case($_tmp) : $_tmp;
    }
    ...
    {
        $_file_name = $prefix . (empty($this->_depths) ? Helpers::page_beranda : self::_depth($x));
        $_file_path = ABS_VIEW_PATH . DS . $_file_name . '.php';
        include_once file_exists($_file_path) ? $_file_path : ABS_VIEW_PATH . DS . Helpers::page_404 . ($x > 0 ? '-content' : '') . '.php';
    }
